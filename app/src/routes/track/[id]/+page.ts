...
export const load: PageLoad = async ({ params, parent }) => {
	...
	if ('error' in trackJson) {
		console.error('Error loading track page:', trackJson.error);
		error(404, trackJson.error);
	}

	const artistID = trackJson.track.artist_id;
	...
	if (trackJson.track.album_id) {
		const albumResponse = await apiClient.api.albums[':id'].$get({
			param: {
				id: trackJson.track.album_id
			}
		});
		const albumJson = await albumResponse.json();
		if ('error' in albumJson) {
			console.error('Error loading track page:', albumJson.error);
			error(404, albumJson.error);
		}
	...
	const recommendedTracksResponse = await apiClient.api.recommendations.tracks.$get({
		query: {
			type: 'similar',
			seedTrackId: trackJson.track.id
		}
	});
