...
export async function POST({ cookies, locals }) {
	...
	try {
		...
		console.log("Token exp:", decodedToken?.exp ? new Date(decodedToken.exp * 1000).toISOString() : 'N/A');
	} catch (decodeErr) {
		console.error("Token decode error:", decodeErr);
	}

	try {
		// Clean the token by removing all whitespace (newlines, spaces, tabs)
		const cleanToken = locals.token.replace(/\s+/g, '');
		
		console.log(`Using clean token for refresh (whitespace removed)`);
		
		// Call the refresh_token RPC function to get a new token
		let { data, error: refreshError } = await databaseClient.rpc("refresh_token", {
			current_token: cleanToken,
		});
