...
export const load = (async ({ parent }) => {
	...
	const { data: recommendedTracks } = await supabase
		.from('tracks')
		.select('*, artist: artists(artist_name)')
		.limit(10);
	const { data: recommendedAlbums } = await supabase.from('albums').select('*').limit(10);
	const { data: recommendedPlaylists } = await supabase.from('playlists').select('*, tracks(*), creator:users!playlists_user_id_fkey(*)').limit(10);
