...
export const getGenreBySlug = async (slug: string): Promise<Genre | null> => {
  ...
  try {
    ...
    const { data: allGenres, error: allGenresError } = await db
      .from('genres')
      .select('*');
...
export const getArtistsByGenre = async (
  ...
  const db = getClient();
  const { limit = 20, offset = 0, sortBy = 'name', sortOrder = 'asc' } = params;
 ...
 */
export const getGenreContentCounts = async (genreId: string): Promise<{
  trackCount: number;
  artistCount: number;
  albumCount: number;
}> => {
