...
export const getEarningsData = async (
	...
	try {
		...
		const { data: previousEarnings, error: previousError } = await db.rpc(
		...
		const currentEarnings =
			periodData?.reduce((sum: number, item) => sum + parseFloat(item.amount.toString()), 0) || 0;
...
export const getGeoDistribution = async (
	...
	try {
		...
		const total = data.reduce(
			(sum: number, item) => sum + parseInt(item.play_count.toString()),
			0
		);
