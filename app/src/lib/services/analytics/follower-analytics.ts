...
export const getFollowerStats = async (
  ...
  try {
    ...
    const { data: followersData, error: followersError } = await db.rpc('get_artist_followers_count', {
      artist_id: artistId
    });
    ...
    const { data: newFollowersData, error: newFollowersError } = await db.rpc('get_followers_by_period', {
      artist_id: artistId,
    ...
    const currentPeriodFollowers = newFollowersData?.reduce(
      (sum: number, item: any) => sum + parseInt(item.count.toString()), 
      0
    ) || 0;
    ...
    return {
      totalFollowers,
      followersTrend,
      newFollowers: newFollowersData ? newFollowersData.map((d: any) => ({
        period: d.period,
