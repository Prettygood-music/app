 ...
 */
export const getTrafficSources = async (artistId: string, period: AnalyticsPeriod = 'all') => {
	const db = getClient();
	const { startDate, endDate } = getDateRange(period);
	...
	try {
		...
		const total = data.reduce((sum: number, item) => sum + parseInt(item.count.toString()), 0);
...
export const getTracksPerformance = async (
	...
	try {
      ...
      `
		...
		const { data: currentPlays, error: currentPlaysError } = await db.rpc(
		...
		const { data: previousPlays, error: previousPlaysError } = await db.rpc(
			'get_track_plays_for_period',
			{
				track_ids: trackIds,
		...
		const tracksWithPlayCounts = tracks.map((track) => {
			const currentPlayCount = parseInt(
				currentPlays?.find((p) => p.track_id === track.id)?.count?.toString() || '0'
			);

			const previousPlayCount = parseInt(
				previousPlays?.find((p) => p.track_id === track.id)?.count?.toString() || '0'
			);
