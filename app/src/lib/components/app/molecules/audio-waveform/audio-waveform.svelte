		...
		{:else if $waveform.length > 0}
			{#each $waveform as amplitude, i}
				...
				<div
					...
					class:current={progress >= (i / $waveform.length) * 100 &&
						progress < ((i + 1) / $waveform.length) * 100}
					style="height: {amplitude}%"
				></div>
			{/each}

			<div class="progress-indicator" style="left: {getProgressPercentage()}%"></div>
