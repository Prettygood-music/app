<script lang="ts">
  ...
  let { 
    ...
    onSubmit: (data: TrackCreationData) => Promise<void>;
    initialValues?: Partial<TrackCreationData>;
    albums?: Array<{ id: string; title: string }>;
  }>();
  
  // Available genres
  const availableGenres = [
    'Pop', 'Rock', 'Hip-Hop', 'Rap', 'R&B', 'Electronic', 
    'Dance', 'Jazz', 'Blues', 'Country', 'Folk', 'Classical', 
    'Metal', 'Punk', 'Reggae', 'Soul', 'Funk', 'Ambient',
    'World', 'Latin', 'Indie', 'Alternative', 'Experimental'
  ];
  ...
  async function handleSubmit() {
    ...
    // Validate form
    try {
      const parsedData = trackCreationSchema.parse(formData);
...
<form method="POST" on:submit|preventDefault={handleSubmit} class="space-y-6">
  <div class="space-y-6">
    <div>
      <h2 class="text-xl font-semibold mb-4">Upload Track</h2>
      <p class="text-muted-foreground mb-4">Complete the form below to upload your track. Fields marked with * are required.</p>
    </div>
    
    <!-- Audio File Upload -->
    <div class="space-y-2">
      <Label for="audio_file" class="block">Audio File <span class="text-destructive">*</span></Label>
      <FileInput 
        id="audio_file" 
        name="audio_file"
        accept="audio/*" 
      ...
      {#if isAudioLoading}
        <div class="text-sm text-muted-foreground mt-2">Loading audio...</div>
      {:else if audioPreview}
        <Card class="mt-2">
          <CardContent class="p-4">
            <audio controls src={audioPreview} class="w-full"></audio>
            <p class="text-sm mt-2">Duration: {formatTime(audioDuration)}</p>
          </CardContent>
        </Card>
      {/if}
    </div>
    ...
    <div class="space-y-2">
      ...
      {#if imagePreview}
        <div class="mt-2 w-40 h-40 rounded overflow-hidden border border-border">
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2 mb-2">
        {#each formData.genre as genre}
          <Badge variant="secondary" class="gap-1 pr-1">
            ...
            <button 
              type="button" 
              class="ml-1 rounded-full hover:bg-muted p-1" 
        ...
        {#if formData.genre.length === 0}
          <p class="text-sm text-muted-foreground">No genres selected</p>
        {/if}
      </div>
      ...
      <div class="flex gap-2">
        <select 
          ...
          class="flex-1 px-3 py-2 bg-background border border-input rounded-md"
        >
        ...
        <Button type="button" variant="outline" on:click={addGenre} disabled={!selectedGenre}>Add</Button>
      </div>
    </div>
    ...
    <div class="space-y-2">
      ...
      />
      <p class="text-xs text-muted-foreground">The International Standard Recording Code (ISRC) uniquely identifies recordings</p>
    </div>
