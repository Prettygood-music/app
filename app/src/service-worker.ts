...
const ROUTES_TO_CACHE = ['/', '/search', '/library', '/login', '/offline', '/install'];
			...
			return fetch(event.request)
				...
				.catch(() => {
					...
					if (event.request.destination === 'image') {
						...
						return caches.match('/web-app-manifest-192x192.png').then((placeholderResponse) => {
							return (
								placeholderResponse ||
...
sw.addEventListener('push', (event) => {
	...
	event.waitUntil(sw.registration.showNotification(data.title || 'prettygood.music', options));
});
